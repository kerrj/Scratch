 ...
 */
public class ScriptBlockSurfaceView extends SurfaceView implements SurfaceHolder.Callback{
    ...
    public ScriptBlockSurfaceView(Context context) {
        super(context);
        getHolder().addCallback(this);
    }

    public ScriptBlockSurfaceView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        getHolder().addCallback(this);
    }

    public ScriptBlockSurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
        getHolder().addCallback(this);
    }
        ...
        public void run(){
            while(running){
                try {
                    ...
                    synchronized(ScriptingManager.getCurrentScriptBlocks()) {
                        for (Iterator<ScriptBlock> iterator = ScriptingManager.getCurrentScriptBlocks().iterator(); iterator.hasNext(); ) {
                            ScriptBlock s = iterator.next();
                            s.draw(mCanvas);
                        }
                    }
//                    for(ScriptBlock s:ScriptingManager.getScriptBlocks()){
//                        s.draw(mCanvas);
//                    }
    ...
    public boolean onTouchEvent(MotionEvent e){
        switch (e.getActionMasked()){
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                lastActionCoordinates[0]=e.getX();
                lastActionCoordinates[1]=e.getY();
                focus= ScriptingManager.getTouchFocus(actionDownCoordinates[0], actionDownCoordinates[1]);
                if(focus!=null){
